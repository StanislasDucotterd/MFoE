...
def test_hyperparameter(lamb, sigma):
    ...
    for image in images:
        x_gt = torch.load(os.path.join(data_folder, image, 'img.pth'), weights_only=True).to(device)
        y = torch.load(os.path.join(data_folder, image, f'y{kernel_id}_{noise_type}_noise.pth'), weights_only=True).to(device)
        ...
        if testing: starter.record()
        pred = SAGD(y, y, model, sigma * torch.ones(1, 1, 1, 1, device=device), max_iter, tol)[0]
        if testing: 
            ender.record()
            torch.cuda.synchronize()
            times.append(starter.elapsed_time(ender) / 1000)
            torch.save(pred.cpu(), os.path.join(data_folder, image, f'pred_{kernel_id}_{noise_type}_mfoe.pt'))
        model.lamb.data = model.lamb - math.log(lamb) 
...
H = torch.nn.Conv2d(1, 1, kernel_size=25, padding=12, bias=False, padding_mode='circular')
H.weight.data = kernel  
Ht = torch.nn.Conv2d(1, 1, kernel_size=25, padding=12, bias=False, padding_mode='circular')
...
infos = torch.load('../../trained_models/' + model_name + '/checkpoints/checkpoint_5000.pth', map_location='cpu', weights_only=True)
config = infos['config']
...
data_folder = 'val_data/'
images = os.listdir(data_folder)
best_lambda, best_sigma = tune_hyperparameters(test_hyperparameter, best_lambda, best_sigma)
...
print(model_name)
print(f'Best PSNR is {final_psnr:.6} with Lambda={best_lambda:.6} and Sigma={best_sigma:.6}')